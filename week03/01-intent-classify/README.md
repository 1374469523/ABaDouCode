...
```bash
curl -X POST 'http://0.0.0.0:8000/v1/text-cls/bert' \
  -H 'Content-Type: application/json' \
...
```json
{
  "request_id": "原请求ID",
  "request_text": "原请求文本",
  "classify_result": "分类结果（字符串 或 字符串列表）",
  "classify_time": 0.023,
  "error_msg": "ok"
}
```
...
# 正则接口压测
ab -n 100 -c 100 -p data.json -T 'application/json' \
  -H 'accept: application/json' 'http://0.0.0.0:8000/v1/text-cls/regex'

# TFIDF 接口压测
ab -n 100 -c 100 -p data.json -T 'application/json' \
  -H 'accept: application/json' 'http://0.0.0.0:8000/v1/text-cls/tfidf'

# BERT 接口压测
ab -n 100 -c 100 -p data.json -T 'application/json' \
  -H 'accept: application/json' 'http://0.0.0.0:8000/v1/text-cls/bert'
```
