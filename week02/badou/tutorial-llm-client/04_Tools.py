...
# 文本（非结构化） -》 llm tool call -》 结构化函数调用

messages = [
    {"role": "system", "content": "你是智能客服助手，可根据需要使用工具回答用户问题。当前日期：2026-07-13。"},
    {"role": "user", "content": "帮我搜索 会员等级 相关的文档"},
    #  晴，28~35°C，南风 2 级

    # 结合上述用户提问 + 工具查询结果 -》 总结 回答
]

response = client.chat.completions.create(
    model="deepseek-v4-flash",
    messages=messages,
    tools=TOOLS,  # 工具（函数定义）定义给大模型，让大模型不是直接返回文本，选择其中一个或多个得到调用方式
    temperature=0.0,
)
...
for turn in range(max_turns):
    ...
    choice = response.choices[0]
    print(choice)
    msg = choice.message # 大模型的返回 由 tool_call 也有正常对话的返回。
...
response = client.chat.completions.create(
    ...
    tools=TOOLS,
    tool_choice="none", # 大模型自己推理调用哪一个工具
    temperature=0.0,
)
...
response = client.chat.completions.create(
    ...
    tools=TOOLS,
    tool_choice={"type": "function", "function": {"name": "calculate"}}, # 强制调用某个工具
    temperature=0.0,
